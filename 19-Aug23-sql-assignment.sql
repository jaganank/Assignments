...
insert into countries values(8,'Morroco');
...
/* Q26. Write an SQL query to get the names of products 
...
select user_id,name,mail from users where mail regexp '^[A-Za-z]+[A-Za-z0-9\_\.\-]*@leetcode[\.]com$' order by user_id;
...
insert into content values(5,'Cindrella','Y','Movies');

select c.title from content c join tvprogram t on c.content_id = t.content_id
where month(t.program_date) = 6 and c.kids_content = 'Y';
...
insert into User values(4,'Donald');
insert into User values(7,'Lee');
insert into User values(13,'Jonathan');
insert into User values(19,'Elvis');
...
select u.u_name,sum(distance) as travelled_distance from userstable u
left join ridestable r
on u.id = r.user_id
group by u_name 
order by travelled_distance desc,u.u_name;
...
/* Q38. Write an SQL query to find the id and the name of all students who are 
...
select unitsold.product_id,round(sum(units * price)/sum(units),2) as average_price from unitsold 
inner join prices on unitsold.product_id = prices.product_id
where unitsold.purchase_date between prices.start_date and prices.end_date
...
insert into products values(4,'LC-Shirt',4,10,20);
...
select cust_id as Customer_id from Customer 
GROUP BY cust_id
